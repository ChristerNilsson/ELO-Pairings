...
* ELO Performance is used instead of adding game scores (1,0.5 and 0)
* Similar ELO Performance has higher priority then similar score
* The ELO Performance is used as the primary measurement and reflects the coming official ELO update.
...
* •• never happens
* o•• may happen 
* #blacks minus #whites is always one of -1, 0 and 1

# Pages

The are four pages:
* Standings (The final listing ordered by ELO Performance)
* Tables    (shows where to sit and also user for entering results)
...
* Columns
	...
	* Chg - the total ELO Performance change in all rounds
		* Most changes are approx: Win = K/2 Draw = 0 Loss = -K/2 as the elo differences are small.
	* Perf = Elo + Chg - the players Performance
